...
fetch("https://striveschool-api.herokuapp.com/books")
  ...
  .then((data) => {
    ...
    data.forEach((book) => {
      ...
      newBook.innerHTML = new Card(book.img, book.title, book.price, book.asin).toString();
      newCol.appendChild(newBook);
      container.appendChild(newCol);

      document.getElementById(`discard-${book.asin}`).addEventListener("click", function (e) {
        e.target.parentElement.parentElement.parentElement.style.display = "none";
      });

      document.getElementById(`buy-${book.asin}`).addEventListener("click", function (e) {
